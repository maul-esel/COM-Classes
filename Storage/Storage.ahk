...
Documentation:
	- *class documentation* (http://maul-esel.github.com/COM-Classes/AHK_Lv1.1/Storage)
	...
	*/
	CopyTo(iidExclude, nameExclude, dest, iidExcludeCount = 0)
	{
		...
		if IsObject(iidExclude)
		{
			...
			{
				iid := iidExclude[A_Index]
				if iid is integer
					CCFramework.CopyMemory(iid, &mem1 + (A_Index - 1) * 16, 16)
				else
					CCFramework.String2GUID(iid, &mem1 + (A_Index - 1) * 16)
			}
			iidExclude := &mem1
		}
